...
classdef alink < handle
    properties
        % kinematic parameters
        theta % kinematic: link angle
        d     % kinematic: link offset
        dels  % kinematic: link offset
        alpha % kinematic: link twist
        a     % kinematic: link length
    ...
    methods
        function l = alink(varargin)
            ...
            elseif nargin == 1 && isa(varargin{1}, 'Link')
                ...
                opt.convention = {'standard', 'modified','enhanced'};
                opt.sym = false;
                opt.flip = false;
                ...
                opt.theta = varargin{1}(4);
                opt.a = varargin{1}(2);
                opt.d = varargin{1}(5);
                opt.alpha = varargin{1}(1);
                opt.dels = varargin{1}(3);
...
%                     l.alpha = dh(4);
                ...
                if strcmp(opt.convention, 'modified')
                    l.mdh = 1;
                elseif strcmp(opt.convention, 'enhanced')
                    l.mdh = 2;
                else
                    l.mdh = 0;
                end
        ...
        function T = A(L, q)
            ...
            elseif L.mdh == 2
                	...
                	T =    [	ct	-st	0	L.a
                        st*ca	ct*ca	-sa	L.dels*ca-sa*d
                        st*sa	ct*sa	ca	L.dels*sa+ca*d
                        0	0	0	1];
            else
                % modified DH
